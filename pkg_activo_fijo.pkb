CREATE OR REPLACE PACKAGE BODY PEVISA.pkg_activo_fijo AS
    ...
    PROCEDURE reclasifica(
        p_cod_activo_fijo     activo_fijo_reclasifica.cod_activo_fijo%TYPE
    ...
    BEGIN
        ...
        mail.texto :=
            RTRIM(mail.texto) || 'Asiento Contable: ' || asiento.ano || s || asiento.mes || s || asiento.libro || s || asiento.voucher || CHR(10);
    ...
    FUNCTION validacion_ok(af activo_fijo%ROWTYPE, valor otm.t_valor)
        RETURN BOOLEAN IS
    BEGIN
        ...
        IF NVL(valor.soles, 0) = 0
           OR NVL(valor.dolares, 0) = 0 THEN
        ...
        IF NOT esta_en_almacen(af.cod_activo_fijo) THEN
            raise_application_error(
                pkg_activo_fijo_err.en_no_esta_en_almacen
    ...
    FUNCTION valor_ingreso_almacen(p_afijo activo_fijo.cod_activo_fijo%TYPE)
        RETURN otm.t_valor IS
        valor otm.t_valor;
    BEGIN
    ...
    FUNCTION fecha_adquisicion(caf activo_fijo.cod_activo_fijo%TYPE)
        RETURN DATE IS
        fch          DATE;
        af           activo_fijo%ROWTYPE;
        es_nacional  BOOLEAN;
    ...
    BEGIN
        ...
        ELSIF es_importado THEN
            SELECT p.fecha
            INTO   fch
            FROM   lg_pedjam p JOIN lg_itemjam i ON p.num_importa = i.num_importa
            WHERE  i.cod_art = caf;
        END IF;
    ...
    FUNCTION fecha_ingreso_almacen(caf activo_fijo.cod_activo_fijo%TYPE)
        RETURN DATE IS
        fch DATE;
    BEGIN
        SELECT g.fch_transac
        INTO   fch
        FROM   kardex_g g
               JOIN kardex_d d
    ...
    FUNCTION nuevo_codigo(p_padre activo_fijo.cod_adicion%TYPE, p_subclase activo_fijo.cod_subclase%TYPE)
        RETURN VARCHAR2 IS
        correlativo    PLS_INTEGER := 0;
        codigo         pkg_types.maxvarchar2;
    ...
    BEGIN
        BEGIN
            ...
            WHERE  table_name = 'ACTIVO_FIJO'
            AND    column_name = 'COD_ACTIVO_FIJO';
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                raise_application_error(-20001, 'No se encontro la tabla ACTIVO_FIJO columna COD_ACTIVO_FIJO');
        END;

        correlativo := correlativo_subclase(p_padre, p_subclase);
        codigo := p_padre || ' ' || p_subclase || correlativo;
        longitud := LENGTH(codigo);

        IF longitud > longitud_tabla THEN
            raise_application_error(-20002, 'La longitud del codigo ' || codigo || ' es mayor a ' || longitud_tabla || ' caracteres');
        END IF;
